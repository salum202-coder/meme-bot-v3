...
def open_position(position: dict[str, Any]) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            """
            INSERT INTO paper_positions(address, symbol, entry_price, quantity, allocated_capital, stop_loss, take_profit,
            trailing_stop_percent, highest_price, status, opened_at)
            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                position["address"],
...
def update_highest_price(position_id: int, highest_price: float) -> None:
    with get_conn() as conn:
        conn.execute("UPDATE paper_positions SET highest_price = ? WHERE id = ?", (highest_price, position_id))
        conn.commit()
